...
// 下面%%之后的是bison的语法规则部分，语法规则使用的是BNF文法。BNF文法是上下文无关文法（Context-Free Grammar，CFG）的标准格式。
// 注意：
//     没有任何声明的记号的语法符号必须规则的左边出现至少一次。如果一个语法符号既不是声明过的记号，也没有出现在任何规则的左边，它将导致出现一个未声明的错误。
//
%%
